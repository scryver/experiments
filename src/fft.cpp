...
DRAW_IMAGE(draw_image)
{
    ...
    if (!state->initialized)
    {
        ...
        fftState->origSignal = arena_allocate_array(gMemoryArena, v2, fftState->origSignalCount, default_memory_alloc());
        fftState->dftSignal = arena_allocate_array(gMemoryArena, Complex32, fftState->origSignalCount, default_memory_alloc());
        fftState->reconstructSignal = arena_allocate_array(gMemoryArena, v2, fftState->origSignalCount, default_memory_alloc());
    ...
    TempArenaMemory tempMem = begin_temporary_memory(&fftState->drawArena);
    ...
    Complex32 *idftBuf = arena_allocate_array(&fftState->drawArena, Complex32, fftState->origSignalCount, default_memory_alloc());
    ...
    f32 oneOverCount = 1.0f / (f32)fftState->origSignalCount;
    v3 *dftSig = arena_allocate_array(&fftState->drawArena, v3, fftState->origSignalCount, default_memory_alloc());
    v3 *dftLogSig = arena_allocate_array(&fftState->drawArena, v3, fftState->origSignalCount, default_memory_alloc());
    for (u32 idx = 0; idx < fftState->origSignalCount; ++idx)
    {
        ...
        logPoint->y = 20.0f * log32(point->y);
