...
DRAW_IMAGE(draw_image)
{
    ...
    if (!state->initialized)
    {
        ...
        neuronCalc->inputs = arena_allocate_array(gMemoryArena, f32, neuronCalc->inputCount, default_memory_alloc());
 ...
 */
        ...
        train.inputCount = 2;
        train.outputCount = 1;
        train.inputs = arena_allocate_array(gMemoryArena, f32, 2, default_memory_alloc());
        train.outputs = arena_allocate_array(gMemoryArena, f32, 1, default_memory_alloc());
        train.inputs[0] = 0.5f;
        train.inputs[1] = 0.7f;
        train.outputs[0] = 0.5f;

        f32 *deltaWeights = arena_allocate_array(gMemoryArena, f32, net->totalWeights, default_memory_alloc());
        f32 *deltaBiases  = arena_allocate_array(gMemoryArena, f32, net->totalNeurons, default_memory_alloc());
#if 1
        ...
        f32 *errors = arena_allocate_array(gMemoryArena, f32, net->totalNeurons, default_memory_alloc());
        f32 *nextErr = arena_allocate_array(gMemoryArena, f32, net->totalNeurons, default_memory_alloc());
