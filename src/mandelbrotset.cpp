...
DRAW_IMAGE(draw_image)
{
    ...
    if (!state->initialized)
    {
        ...
        mandelState->palette = allocate_array(v4, mandelState->paletteCount);
    ...
    mandelState->ui.clicked = mouse.mouseDowns;
        ...
        ((mandelState->constant.y + 5.0f * SCALING) >= at.y))
    {
        if ((mouse.mouseDowns & Mouse_Left) &&
            !(mandelState->prevMouseDown & Mouse_Left))
        {
    ...
    //
    mandelState->prevMouseDown = mouse.mouseDowns;
