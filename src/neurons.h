                    ...
                    u32 hiddenDepth, u32 *hiddenCounts, u32 outputCount)
{
    ...
    network->layerSizes = arena_allocate_array(&network->arena, u32, network->layerCount, default_memory_alloc());
    ...
    network->hidden = arena_allocate_array(&network->arena, f32, network->totalNeurons, default_memory_alloc());
    network->biases = arena_allocate_array(&network->arena, f32, network->totalNeurons, default_memory_alloc());
    ...
    network->weights = arena_allocate_array(&network->arena, f32, network->totalWeights, default_memory_alloc());

    network->outputs = network->hidden + network->totalNeurons - outputCount;

    network->trainErrorsA = arena_allocate_array(&network->arena, f32, maxErrorLen, default_memory_alloc());
    network->trainErrorsB = arena_allocate_array(&network->arena, f32, maxErrorLen, default_memory_alloc());
    network->trainGradient = arena_allocate_array(&network->arena, f32, maxErrorLen, default_memory_alloc());
}

internal void
destroy_neural_network(Neural *network)
{
    arena_deallocate_all(&network->arena);
}
...
internal void
back_propagate(MemoryArena *memoryArena,
               ...
               NeuronActivate *activate = activation_function, void *user = 0)
{
    ...
    TempArenaMemory tempMem = begin_temporary_memory(memoryArena);
    ...
    f32 *temp = arena_allocate_array(memoryArena, f32, totalNeurons, default_memory_alloc());
    f32 *temp2 = arena_allocate_array(memoryArena, f32, totalNeurons, default_memory_alloc());
...
internal void
update_mini_batch(MemoryArena *memoryArena, u32 batchSize, Training *training, f32 learningRate,
                  f32 lambda, u32 totalTrainingCount,
                  u32 layerCount, u32 *layerSizes, f32 *hidden, f32 *weights, f32 *biases,
                  u32 totalNeurons, u32 totalWeights, b32 softmax = false,
                  NeuronActivate *activation = activation_function, void *user = 0)
{
    TempArenaMemory temp = begin_temporary_memory(memoryArena);

    f32 *nw = arena_allocate_array(memoryArena, f32, totalWeights, default_memory_alloc());
    f32 *nb = arena_allocate_array(memoryArena, f32, totalNeurons, default_memory_alloc());
