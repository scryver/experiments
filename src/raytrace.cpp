...
internal RayBundle
init_ray_bundle(v2 pos, f32 angleOfView, u32 rayCount)
{
    RayBundle result = {};
    result.pos = pos;
    result.heading = 0;
...
internal void
rotate_ray_bundle(RayBundle *bundle, f32 angle)
{
    bundle->heading += angle;
    for (u32 rayIdx = 0; rayIdx < bundle->rayCount; ++rayIdx) {
        Ray *ray = bundle->rays + rayIdx;
        ray->dir = rotate(ray->dir, polar_to_cartesian(1.0f, angle));
    }
}
...
internal void
ray_bundle_intersects_walls(Image *image, RayBundle bundle, u32 wallCount, Boundary *walls, f32 *distSqsResult)
{
    for (u32 rayIdx = 0; rayIdx < bundle.rayCount; ++rayIdx) {
        Ray ray = bundle.rays[rayIdx];
...
DRAW_IMAGE(draw_image)
{
    ...
    if (!state->initialized)
    {
        ...
        for (u32 wallIdx = 4; wallIdx < basics->wallCount; ++wallIdx) {
            ...
            wall->start.y = random_unilateral(&basics->randomizer) * size.y * 0.5f;
