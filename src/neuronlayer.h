...
init_neural_network(NeuralCake *network, u32 layers)
{
    ...
    network->layers = arena_allocate_array(&network->arena, NeuralLayer, layers, default_memory_alloc());
...
finish_network(NeuralCake *network)
{
    ...
    f32 *allTheValues = arena_allocate_array(&network->arena, f32,
                                             neuronCount + weightCount + biasCount + poolIdxCount, default_memory_alloc());
...
back_propagate(NeuralCake *network, Training *training, f32 *deltaWeights, f32 *deltaBiases)
{
    ...
    TempArenaMemory tempMem = begin_temporary_memory(&network->arena);
    ...
    f32 *error = arena_allocate_array(&network->arena, f32, maxNeurons, default_memory_alloc());
    f32 *prevError = arena_allocate_array(&network->arena, f32, maxNeurons, default_memory_alloc());
...
{
    TempArenaMemory temp = begin_temporary_memory(&network->arena);

    f32 *nw = arena_allocate_array(&network->arena, f32, network->totalWeights, default_memory_alloc());
    f32 *nb = arena_allocate_array(&network->arena, f32, network->totalBiases, default_memory_alloc());
