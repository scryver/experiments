...
init_flow_field(FlowField *field, u32 width, u32 height, u32 tileSize)
{
    ...
    field->grid = arena_allocate_array(gMemoryArena, v2, field->size.x * field->size.y, default_memory_alloc());
}
...
DRAW_IMAGE(draw_image)
{
    ...
    if (!state->initialized)
    {
        ...
        flow->vehicles = arena_allocate_array(gMemoryArena, Vehicle, flow->vehicleCount, default_memory_alloc());
