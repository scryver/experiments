...
{
    s32 col = maximum(0, minimum(field->size.x - 1, round(position.x / (f32)field->tileSize)));
    s32 row = maximum(0, minimum(field->size.y - 1, round(position.y / (f32)field->tileSize)));
