...
DRAW_IMAGE(draw_image)
{
    ...
    if (!state->initialized)
    {
        ...
        hilbert->hilbertColours = arena_allocate_array(&hilbert->arena, v4, hilbert->maxPointCount);
        hilbert->points = arena_allocate_array(&hilbert->arena, ColouredPoint, hilbert->maxPointCount);
    ...
    if ((mouse.mouseDowns & Mouse_Left) &&
        !(hilbert->prevMouseDown & Mouse_Left))
    {
    ...
    hilbert->prevMouseDown = mouse.mouseDowns;
    hilbert->seconds += dt;
    ++hilbert->ticks;
