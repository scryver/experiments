...
global MemoryArena gArena_;
global MemoryArena *gArena = &gArena_;
...
add_weights_old(u32 multCount, u32 transpCount, f32 *multiplier, f32 *transposer, f32 *weights)
{
    ...
    TempArenaMemory tempMem = begin_temporary_memory(gArena);

    f32 *tempArray = arena_allocate_array(gArena, f32, transpCount, default_memory_alloc());
    f32 *tempWeights = arena_allocate_array(gArena, f32, multCount * colTrans, default_memory_alloc());
...
transpose_multiply_old(u32 outputCount, u32 inputCount, f32 *transposedM, f32 *current, f32 *next)
{
    ...
    f32 *tempWeights = arena_allocate_array(gArena, f32, rowTrans * colTrans, default_memory_alloc());
